 ...
 * @param {DocsToUiConfig} config - The configuration for the conversion
 * @requires module:BuildHtml
 * @fires module:BuildHtml
 */
class DocsToUi {
  ...
  public title: string
  public htmlConfig: htmlConfig //TODO: Move this into DocsToUiConfig
   ...
   */
  constructor(LogLevel: Logging_config['level']['value'], comments: Comments[], config: DocsToUiConfig) {
    //---------------------------------
    //-- Class properties
    this.config = config
    
    this.LogLevel = 1 // defaults as fatal
    this.comments = []
    this.files = []
    this.namespaces = []
    this.modules = []
  ...
  __init__(LogLevel: Logging_config['level']['value'], comments: Comments[], config: DocsToUiConfig) {
    ...
    if(this.LogLevel > 0 ) console.log('TODO: Onboard DocsToUi.__init__ function.')
    if(this.LogLevel > 4 )console.log('logLevel:', LogLevel, 'config.keys: ', Object.keys(config))
  }
    ...
    comments &&
      comments.map((item: Comments) => {
        processedData.push({
          ...
          isRootItem: item.comments?.[this.config.contentGroupParent]?.[0]?.description ? true : false,
  ...
  /**
   * Generate Header, Nav, and Nav-Links for each Content-Group (this.config.contentGroupParent).
   *
   * @function buildHeader
   * @access private
   * @memberof module:DocsToUi
   * @summary Generate Header, Nav, and Nav-Links for each Content-Group (this.config.contentGroupParent).
   * @description Builds the header and nav elements. The builds a link within nav based on each Content-Group (this.config.contentGroupParent).
  ...
  buildHeader(ElementsProcessed: ElementsProcessed): ElementsProcessed {
    ...
    // 4. Get all Content-Groups (this.config.contentGroupParent), and create link in header nav each. (Mapping through, and if match pushing to NavHeaderList children).
    let firstTab = true //-- Default active tab styling.
    this.contentGroups.map((item: CommentsProcessed) => {
      let parentGroupKey:keyof CommentsProcessed = this.config.contentGroupParent as keyof CommentsProcessed
      // console.log('item[this.config.contentGroupParent]: ', item[this.config.contentGroupParent as keyof CommentsProcessed])
      // console.log('type cast', this.config.contentGroupParent as keyof CommentsProcessed)
      
      // 4.1. Get the key for the parent group to look for content group parent match.
      Object.keys(item).map((property) => {
        if(property.includes(this.config.contentGroupParent)){
          parentGroupKey = property as keyof CommentsProcessed
        }
      })

      console.log('item?.[parentGroupKey: ', item?.[parentGroupKey])
        
      // If the item has a parent group, create the link.
      if (item?.[parentGroupKey] ) {
        // 5. Create the main nav elements for each this.config.contentGroupParent
        const NavHeaderLink: Element = {
          ...
          content: [
            `${item?.[parentGroupKey] ? `${item?.[parentGroupKey]}\n` : ''}`,
  ...
  buildGroupContentWrapper(ElementsProcessed: ElementsProcessed): ElementsProcessed {
    ...
    //TODO: needs to be config.contentGroupParent, not namespaces
    this.contentGroups.map((item: CommentsProcessed) => {
      if (item.namespaces.length > 0) {
          ...
          Elements: [
            {
              ...
              description: `Div within 'main' for Group (this.config.contentGroupParent) '${item.namespaces[0]}'. Holds the tab-strip-nav and all GroupContentWrapper (which holds GroupContent).`,
              elementType: 'div',
              classList: [
        ...
        const GroupContent: Element = {
          id: randomUUID(),
          parent: item.id,
          description: `Wrapper to hold content within group-content-wrapper, for group (this.config.contentGroupParent) '${item.namespaces[0]}'.`,
          elementType: 'div',
          classList: ['border-solid border-2 border-t-0 rounded-b-xl h-full p-x-4 p-b-4', 'overflow-hidden'],
          dataAttributes: {
        ...
        // 4. Build the tab strip nav for the content-group (this.config.contentGroupParent).
        GroupContent.children.push(
          // Builds overview tab first, then the rest of the tabs are members of the content-group (this.config.contentGroupParent).
          this.buildTabStripNav(item, item.namespaces[0], item.id, tabStripNavId, tabStripNavListId),
        )

        // 5. Build the Content-Group children.
        GroupContent.children.push(
          // Builds the Overview content first, then the rest of the tabs are members of the content-group (this.config.contentGroupParent).
  ...
  buildTabStripNav(
    ...
    const tabStripLinks: Element[] = [
      // 4. Build The overview tab definition, which is always first.
      {
        ...
        dataAttributes: {
          ...
          group:
            group ||
            parentItem.memberOf?.filter(value => value.type == this.config.contentGroupParent)[0]?.description ||
            parentItem.parent?.filter(value => value.type == this.config.contentGroupParent)[0]?.description ||
    ...
    // 5. Define the rest of the tab-strip-nav-links for the content-group (this.config.contentGroupParent).
    ...
    this.processedData.map((item: CommentsProcessed) => {
      ...
      if (item.parent.filter(itemParent => itemParent.id === parentId).length > 0) {
        // 5.2 Build any additional tab-strip-nav-links for the content-group (this.config.contentGroupParent).
